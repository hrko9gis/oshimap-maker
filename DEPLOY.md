...
- デプロイ設定：`wrangler.jsonc` を同梱済み。Cloudflare が本プロジェクトを **Workers（Static Assets）** として作成し `npx wrangler deploy` を実行する構成で、`dist/` を静的アセットとして配信します（`main` を持たない静的専用）。これにより **Vite 5 のままデプロイでき**ます。
- SPA フォールバック：`wrangler.jsonc` の `not_found_handling: "single-page-application"` が担います。深いURL（例 `/:projectId/export`）を直接開いても 404 になりません。**`_redirects` ファイルは同梱しません**（Workers Static Assets では `/* /index.html 200` の catch-all が「無限ループ」と判定されデプロイが失敗するため。SPA処理は上記の設定に一本化）。
...
- **現在の公開URL：https://oshimap-maker.hrko9gis.workers.dev** （Workers Static Assets 方式）。
- 発行された URL を、マップをつくる人に伝える。以後その人は URL を開くだけで作業できます（`PowerShell` 不要）。
...
## Supabase（フェーズ5a）を使う場合の環境変数

クラウド保存（複数端末・共同編集）を有効にするには、Cloudflare のビルド環境変数に次を設定する
（設定手順とスキーマ適用は `SUPABASE.md` を参照）：

| 変数名 | 値 |
|--------|-----|
| `VITE_SUPABASE_URL` | Supabase の Project URL（`https://xxxx.supabase.co`） |
| `VITE_SUPABASE_ANON_KEY` | Supabase の anon public key |

- **`service_role` キーは設定しない**（クライアントに露出させない）。
- これらが**未設定なら、アプリは従来どおり localStorage 動作**（ログイン不要のローカルモード）にフォールバックする。
- Supabase の認証メール（マジックリンク）は本番でカスタムSMTPが必要（`SUPABASE.md` 参照）。

## トラブル時

| 症状 | 対処 |
|------|------|
| 深いURLを開くと 404 | `wrangler.jsonc` に `not_found_handling: "single-page-application"` があるか確認。`_redirects` は追加しないこと（Workers Static Assets では無限ループ判定でデプロイが失敗する）。 |
| `Invalid _redirects configuration: Infinite loop detected` | `dist`/`public` に `_redirects` を置かない。SPA処理は `wrangler.jsonc` の `not_found_handling` に一本化する。 |
